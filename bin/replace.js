...
setDefaultData(config).then(() => {
  ...
  if (fs.lstatSync(entry).isDirectory()) {
    ...
    /(\.vue)|(\.jsx)$/.test(entry) ||
    (config.extra && config.extra.test(entry))
  ) {
    readFile(entry, ++id);
  }
});
    ...
    files.forEach((file) => {
      ...
      if (fs.lstatSync(name).isDirectory()) {
        ...
        if (isInclude(include, file) || isRead) {
          if (
            /(\.vue)|(\.jsx)$/.test(name) ||
            (config.extra && config.extra.test(name))
          ) {
          ...
          generateI18nData().then((data) => {
            fs.writeFile(
              path.resolve(
                process.cwd(),
                config.output
                  ? config.output.replace("en-US", "zh-CN")
                  : "i18n.data.js"
              ),
              "export default " +
                JSON.stringify(
            ...
            fs.writeFile(
              path.resolve(
                process.cwd(),
                config.output
                  ? config.output.replace("zh-CN", "en-US")
                  : "i18n.data.js"
              ),
              "export default " +
                JSON.stringify(
