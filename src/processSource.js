const { globalData } = require("i18n-replace/src/data");
let i18nMap, i18nMap2, prefix, suffix, mode, pluginPrefix, wordPrefix;
...
async function processSource(code = "") {
  ...
  wordPrefix = globalData.wordPrefix;
  let arr;
  let separator;
  if (code.split("\r\n").length > 1) {
    arr = code.split("\r\n");
...
function replaceScript(str = "", data = []) {
  ...
  data.forEach((item) => {
    ...
    if (word == s) {
      s = `${pluginPrefix}.formatMessage({ id: '${wordPrefix}${replace}' })`;
    } else {
      ...
      s =
        ...
        s.replace(
          word,
          "${" +
            `${pluginPrefix}.formatMessage({ id: '${wordPrefix}${replace}' })` +
            "}"
        ) +
        "`";
    }
